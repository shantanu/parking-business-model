...
from model import get_locations, print_model_output
...
def rename_columns_ai4m(df):
    def namechange(name: str):
    ...
    df.rename(namechange, axis="columns", inplace=True)
...
def generate_dataframe(
    ...
    cumulative_locations = [
        ...
        for x in zip(
            *(
                model_output.cumulative_locations
    ...
    cumulative_partners = [
        ...
        for x in zip(
            *(
                (
                    model_output.cumulative_partners
                    if model_output.cumulative_partners
                    else [0] * len(model_output.cumulative_locations)
                )
                for model_output in model_outputs.values()
            )
        )
    ...
    total_cumulative_license_fee_all_models = list(
        accumulate(total_license_fee_per_month_sum)
    )
    print(
        "TOTAL CUMULATIVE LICENSE FEE ALL MODELS: ",
        total_cumulative_license_fee_all_models,
    )

    sales_commission_per_month = get_sales_commission(
        sales_params, total_license_fee_per_month_sum
    )
    ...
    cash_flow_per_month = [
        get_cash_flow(fee, commission, cost)
        for fee, commission, cost in zip(
            total_license_fee_per_month_sum, sales_commission_per_month, cost_per_month
        )
    ]
    ...
    valuation_per_year = get_yearly_valuation(
        valuation_params, total_cumulative_cash_flow
    )
    ...
    monthly_recurring_revenue = [
        total_license_fee_per_month_sum[i * 12 - 1] for i in range(1, MONTHS // 12 + 1)
    ]

    yearly_locations = [
        cumulative_locations[i * 12 - 1] for i in range(1, MONTHS // 12 + 1)
    ]

    yearly_partners = [
        cumulative_partners[i * 12 - 1] for i in range(1, MONTHS // 12 + 1)
    ]
    ...
    # Create DataFrames
    dataframes = [
        model_output_to_dataframe(output, name)
        for name, output in model_outputs.items()
    ]
    ...
    if rename_columns:
        df_partner_business_model = rename_columns_ai4m(df_partner_business_model)

    # Write the concatenated DataFrame to a CSV file
    df_partner_business_model.to_csv(output_path, index=False)

    print(
        f"All DataFrames have been successfully combined and written to {output_path}."
    )
